...
if __name__ == "__main__":
    ...
    config_list = [get_config(have_monitor=True, radio_mode='diaspora')]
        ...
        for app in [noop]:
            ...
            for trial in range(args.trials):
                try:
                    ...
                    db.execute(f"""
                        insert into
                        "{table_name}"(monitor_tag, start_submit, end_submit, returned, connected_workers, tasks_per_trial, task, tag)
                        values (?, ?, ?, ?, ?, ?, ?, ?)""", data
                    )
                    db.commit()
