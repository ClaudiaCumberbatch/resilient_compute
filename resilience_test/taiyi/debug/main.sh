...
# timeout, random
FAILURE_TYPES=("dependency" "failure" "import" "manager-killed" "memory" "ulimit" "worker-killed" "zero-division") # 8
...
for failure_rate in "${FAILURE_RATES[@]}"; do
  for base in "${BASES[@]}"; do
    for failure_type in "${FAILURE_TYPES[@]}"; do
      ...
      echo "Start time: $(date), Configuration: base = $base, TOML_FILE = $TOML_FILE, failure_type = $failure_type, failure_rate = $failure_rate."
